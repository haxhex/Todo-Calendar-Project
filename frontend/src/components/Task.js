...
class Task extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      timeInput: "00:00", // Initialize timeInput state to an empty string
    ...
    selectedTasks.forEach(taskId => {
      ...
      if (taskToCopy) {
        ...
        formattedItem.tags.forEach(tag => {
          formData.append("tags", tag); // Assuming the tag object has a 'text' property
        });
  ...
  refreshList = () => {
    ...
    axios
      ...
      .then(res => {
        ...
        // Sort the todoList array first by due_date in ascending order,
    ...
    <button onClick={this.copyTaskInNewDueDate} className="btn btn-primary"  style={{ marginRight: '5px', backgroundColor: 'orange', border: 'orange', borderRadius: '10rem' }}>کپی</button>
      ...
      <div style={{
        ...
        >
          ...
          <label className="date-margin">تا تاریخ:</label>
          {this.state.endDate ? (
          <JalaliDatePicker
...
handleTimeOperation = (operation) => {
  ...
  if (!timeRegex.test(timeInput)) {
    alert("فرمت زمان ورودی صحیح نیست");
    return;
  }
  ...
  const updatedTodoList = todoList.map((task) => {
    if (selectedTasks.includes(task.id)) {
      const endTime = new Date(`${task.due_date}T${task.end_time || '00:00:00'}`);
      const newEndTime =
        operation === "add"
  ...
  renderItems = () => {
    ...
    return filteredAndCompleted.map(item => {
      ...
      // const [hoursDur, minutesDur, secondsDur] = duration.split(":");
      // const formattedDur = `${hoursDur}:${minutesDur}`;
...
formattedItem.files.forEach((fileObj) => {
  ...
  render() {
    const {isDropdownOpen, completed } = this.state;
    ...
    const sortedTasks = endedNotCompletedTasks.sort((a, b) => new Date(a.due_date) - new Date(b.due_date));
    const lastFiveTasks = sortedTasks.slice(-5);  
    return (
      <>
      <div class="my-header">
      <h1 dir="rtl" style={{paddingLeft: '14px'}}>داشبورد</h1>      
      <div
      ...
      >
        ...
        <span className="badge" style={{ backgroundColor: "red", color: "white", borderRadius: "50%", padding: "2px 6px", fontSize: "12px", position: "absolute", bottom: "-3px", right: "-6px" }}>
          {persianJs((endedNotCompletedTasks.length + endingSoonTasks.length).toString()).englishNumber().toString()}
        </span>
        ...
        {isDropdownOpen && (
          <div className="dropdown-content">
            ...
            <p style={{color: 'orange'}}>کارهایی که به زودی موعد آنها فرا میرسد:</p>
            )}
            <ul>
              {endingSoonTasks.map((task) => (
                ...
                <input
                  type="checkbox"
                  checked={completed}
                  onChange={() => this.handleCompleteToggle(task)}
            ...
            {endedNotCompletedTasks.length > 0 && (
            <p style={{color: 'red'}}>کارهایی که موعد آنها فرا رسیده است:</p>
            )}
            <ul>
